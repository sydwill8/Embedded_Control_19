...
unsigned int Counts;
char direction;
unsigned int desired_heading = 1350; //desired_heading_s;
unsigned int PW_CENTER = 2765, PW_MIN = 2028, PW_MAX = 3502;	//Neutral, min, and max PW limit
unsigned int SERVO_PW, actual_heading,	actual_height, neutral_height;
unsigned long temp_servo_pw;
signed int PW_DC;
unsigned int error ;
...
void DC_Speed(void)
{
	...
	//	direction = 'f';
		BI2 =0;
		BI1 =1;
		PW_DC = 1*(-160)/51*AD_value+2800;	//Calculated Pulse width for DC motor
		desired_heading = 1350;
		error_servo();							
		Servo_speed();
		actual_heading = ReadCompass();
