...
unsigned int ReadRanger()
{
	...
	i2c_read_data(addr, 6, Data,2);		//read two bytes, starting at register 6
...
unsigned int ReadCompass()
{
	...
	unsigned char Data[3];		//Data is an array with a length of 3
	unsigned int heading;		//the heading returned in degrees between 0 and 3599
	i2c_read_data(addr, 0 ,Data,1);	//read one byte, starting at reg 0
	i2c_read_data(addr, 4,Data,2);	//read two byte, starting at register at 4
